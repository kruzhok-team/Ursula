...
public partial class CreateTerrain : Node
{
    public static string NAMETERRAIN = "Terrain";

    public static CreateTerrain instance;
	...
	{
		VoxLib.createTerrain ??= this;

		//StartCoroutineCreateTerrain();
	}

	private async void StartCoroutineCreateTerrain(bool randomHeight)
	{
		await ToSignal(GetTree().CreateTimer(0.1f), "timeout");
		GD.Print("1 second passed!");

		if (VoxLib.createTerrain == this)
		{
			ProcCreateTerrainRandom(randomHeight);
		}
	}
    ...
    float[,] SmoothPlateauEdges(float[,] heightMap, double smoothFactor, int sizeSmooth)
	{
    ...
    bool IsPlateauEdge(int x, int y)
    {
		if (mapHeight[x, y] == plateauHeight) return true;
        return false;
    }

    static int CalculateSmoothedValue(float[,] heightMap, int x, int y, double smoothFactor, int sizeSmooth)
    {
        float sum = 0;
        int count = 0;
    ...
    void Generate(Vector3 position, int numChunkX, int numChunkZ)
	{
		VoxLib.RemoveAllChildren(VoxLib.mapManager.navigationRegion3D);
	...
	public float MaxHeightTerrain
	{
		get
		{
			int size = VoxLib.createTerrain.size;

			if (VoxLib.createTerrain.mapHeight == null) return 0;
			...
			for (int i = 0; i < size; i++)
			{
				for (int j = 0; j < size; j++)
				{
					height = (height > VoxLib.createTerrain.mapHeight[i, j]) ? height : VoxLib.createTerrain.mapHeight[i, j];
				}
			}
			height += VoxLib.createTerrain.positionOffset.Y;
			return height;
		}
	}
    ...
    void GenerateEdgesMap()
	{
		VoxLib.RemoveAllChildren(VoxLib.createTerrain);
		...
		for (int n = 0; n < 4; n++)
		{
			...
			VoxLib.createTerrain.AddChild(staticBody);
			float tmpRangeX = 0;
